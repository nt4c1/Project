...
public class DoctorRepositoryImpl implements DoctorRepositoryPort {
    ...
    // TypeReferences for generic list deserialization (fixes Doctor[].class anti-pattern)
    private static final TypeReference<List<Doctor>> DOCTOR_LIST_TYPE = new TypeReference<>() {};
    ...
    private final PreparedStatement selectByLocationText;
    ...
    public DoctorRepositoryImpl(CqlSession session,
        ...
        selectByLocationText = session.prepare(
                "SELECT doctor_id, clinic_id, name, type, specialization, phone, latitude, longitude, is_active " +
                        "FROM doctor_service.doctors_by_location " +
                        "WHERE location_text = ? AND is_active = true AND is_deleted = false ALLOW FILTERING"
        );
    ...
    public void updateLocation(UUID doctorId, UUID clinicId, Location location) {
        ...
        // Soft-delete old geohash entry for this doctor/clinic combo
        ...
        Row profileRow = session.execute(
                selectDoctorProfileForLocation.bind().setUuid("doctor_id", doctorId)
        ).one();
        ...
        // Fix: was caching a Location object under a key that reads back Doctor[] — removed.
        // Location data is embedded in Doctor objects returned by findNearby/findByLocationText.
        clearLocationCache();
    ...
    public Optional<Doctor> findById(UUID doctorId) {
        ...
        Row r = session.execute(
                selectDoctorById.bind().setUuid("doctor_id", doctorId)
        ).one();
    ...
    @Override
    public List<Doctor> findByLocationText(String locationText) {
        String cacheKey = CACHE_LOCATION_PREFIX + "lt:" + locationText;
        try {
            List<Doctor> cached = redisUtil.get(cacheKey, DOCTOR_LIST_TYPE);
            if (cached != null) {
                log.info("Redis cache hit for findByLocationText: {}", cacheKey);
                return cached;
            }
        } catch (Exception e) {
            log.warn("Redis GET failed for findByLocationText: {}", e.getMessage());
        }

        ResultSet    rs   = session.execute(selectByLocationText.bind(locationText));
    ...
    public List<Doctor> findNearby(String geohash, double lat, double lon) {
        ...
        String cacheKey = CACHE_LOCATION_PREFIX + "nb:" + prefix;
        try {
            List<Doctor> cached = redisUtil.get(cacheKey, DOCTOR_LIST_TYPE);
            if (cached != null) {
                log.info("Redis cache hit for findNearby: {}", cacheKey);
                return cached;
            }
        } catch (Exception e) {
        ...
        for (int precision = 6; precision >= 4; precision--) {
            String p     = geohash.substring(0, Math.min(geohash.length(), precision));
            List<Doctor> found = searchInPrefixAndNeighbors(p, lat, lon);
            for (Doctor d : found) {
    ...
    public List<Doctor> findByClinicId(UUID clinicId) {
        ResultSet    rs   = session.execute(selectByClinic.bind()
                .setUuid("clinic_id", clinicId)
                .setBoolean("is_active", true));
        List<Doctor> list = new ArrayList<>();
        for (Row r : rs) {
    ...
    public void updateDoctor(UUID doctorId, String email, String password, String phone) {
        ...
        Row        r         = session.execute(selectDoctorById.bind().setUuid("doctor_id", doctorId)).one();
    ...
    public void deleteDoctor(UUID doctorId, String email, String password) {
        Instant    now = Instant.now();
        ResultSet  rs  = session.execute(selectAppointmentsByDoctor.bind().setUuid("doctor_id", doctorId));
    ...
    /**
     * Clears all location cache entries by scanning for the prefix pattern.
     * Uses SCAN internally (via RedisUtil) — safe for production.
     */
    private void clearLocationCache() {
        ...
        try {
            ...
            if (keys == null || keys.isEmpty()) {
                log.debug("No location cache entries to clear.");
                return;
            }
            // Fix: was firing N independent deleteAsync calls with no coordination.
            // Now deletes all keys in a single loop but logs the count.
            for (String key : keys) {
                redisUtil.deleteAsync(key);
            }
            log.debug("Cleared {} location cache entries.", keys.size());
        } catch (Exception e) {
